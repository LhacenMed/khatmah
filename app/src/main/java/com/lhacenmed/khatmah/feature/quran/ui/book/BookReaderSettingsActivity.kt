...
/**
 * Hosts the reader's settings fragment — Quran Android's `QuranPreferenceActivity` analog.
 * Native toolbar with a platform back arrow; the body is [BookReaderSettingsFragment].
 *
 * Edge-to-edge: the toolbar covers the status-bar strip (surfaceContainer) and the body sits
 * under the nav bar (surface), so each system-bar strip matches the region it overlays.
 */
class BookReaderSettingsActivity : AppCompatActivity() {
