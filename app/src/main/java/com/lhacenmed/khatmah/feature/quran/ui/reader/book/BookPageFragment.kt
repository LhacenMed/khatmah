...
/**
 * Renders one QCF4 page of the reader. The page and its typefaces are decoded off the main thread,
 * then handed to a [BookPageView]. The active riwaya's [Qcf4Repository] is resolved from
...
class BookPageFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        // Landscape: keep the page's portrait proportions at full width and scroll vertically to
        // reveal it. The page info stays at the page's own top/bottom, so it scrolls along.
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        // …then stay in sync: any display change (night/brightness/page-info) updates every live
        // page at once — including offscreen neighbours — so swiping never shows a stale frame.
        viewLifecycleOwner.lifecycleScope.launch {
            combine(
                ReaderTheme.override,
                ReaderPrefs.textBrightness,
                ReaderPrefs.backgroundBrightness,
                ReaderPrefs.showPageInfo,
    ...
    private fun loadPage() {
        ...
        val repo = Qcf4Repository.get(ctx, MushafPrefs.selected.value.riwaya)
    ...
    /**
     * The accent (primary) for the reader's effective theme — the dynamic-colour scheme built for
     * [night], independent of the app's current light/dark state.
     */
